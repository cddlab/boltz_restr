...
class AtomDiffusion(Module):
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        # noisy ~= denoised, and the step_scale extrapolation x = 1.5*denoised - 0.5*noisy
        # collapses to ~denoised (1.5*X - 0.5*X = X), so the output is already on the
        # conformer target without a separate sigma=0 pass. finalize only logs the residual.
        if combined_restr is not None:
